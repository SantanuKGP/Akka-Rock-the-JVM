...
object mailbox {
  ...
  class SupportTicketPriorityMailbox(settings: akka.actor.ActorSystem.Settings, config: Config) extends UnboundedPriorityMailbox(
    PriorityGenerator{
  ...
  def demoControlAwareMailbox(): Unit = {
    ...
    ActorSystem(userGuardian,"DemoControlAware",ConfigFactory.load().getConfig("mailboxes-demo")).withFiniteLifeSpan(2.seconds)
  }
